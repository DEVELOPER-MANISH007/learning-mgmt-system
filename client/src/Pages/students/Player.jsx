...
const Player = () => {
  ...
  const getCourseData = () => {
    const pools = [Array.isArray(enrolledCourses) ? enrolledCourses : [], Array.isArray(allCourses) ? allCourses : []];
    const found = pools.flat().find((course) => course && course._id === courseid);
    setCourseData(found || null);
  };
  useEffect(() => {
    getCourseData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [courseid, enrolledCourses, allCourses]);
              ...
              <YouTube videoId={playerData.lectureUrl.split('/').pop()} opts={{playerVars:{
                ...
                <div className="flex justify-between items-center mt-1">
                  ...
                  <button className="text-blue-600">
                    {playerData && playerData.isCompleted ? "Completed" : "Mark Complete"}
                  </button>
                </div>
            </div>
          )
